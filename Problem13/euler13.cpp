...
int main(){
	...
	for(int c = 99; c >=50; c--){
		for (int r = 0; r < 100; r++){
			colSum[c] += nums[r][c-50];
		}
	}

    //deal with carry over up to the hundreds
	for (int c = 99; c >=3; c--){
		int hunCarry = (int) colSum[c] / 100;
		colSum[c] -= hunCarry * 100;
		int tenCarry = (int) colSum[c] / 10;
		colSum[c] -= tenCarry * 10;
		
		colSum[c-1] += tenCarry;
		colSum[c-2] += hunCarry;
	}

    //find the first ten digits
	int digAmt = 0;
	string tenDigits = "";
	for (int c = 0; c < 100 && digAmt < 10; c++){
		if (colSum[c] != 0 || digAmt > 0){
			digAmt++;
			tenDigits.append(to_string(colSum[c]));
		}
	}
	cout << tenDigits << endl;
	return 0;
